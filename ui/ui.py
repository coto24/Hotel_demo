...
class MenuGaste:
    def __init__(self, gui_master):
        ...
        nachname_neu = Label(self.__window, text='New Last Name', bg="lightsteelblue",font=('arial',9,'bold'),relief=RIDGE)
    ...
    def __Print(self):
        resultLabel = Label(self.__window, text=str(control.print_guests()),bg="lightsteelblue",font=('arial',9,'bold'),relief=RIDGE)
...
class MenuZimmer:
    def __init__(self, gui_master):
        ...
        meerblick = Label(self.__window, text='Seaview [yes/no]',bg="lightsteelblue",font=('arial',9,'bold'),relief=RIDGE)
        ...
        aux0 = Label(self.__window, text='''Please type in one of: ''', bg="lightsteelblue", font=('arial', 9, 'bold'),relief=RIDGE)
        aux0.grid(column=2, row=9, padx=10)
        aux1 = Label(self.__window, text='''  • More expensive than x ''',bg="lightsteelblue",font=('arial',9,'bold'),relief=RIDGE)
        aux1.grid(column=3, row=8, pady=5)
        aux2 = Label(self.__window, text='''  • Less expensive than x ''',bg="lightsteelblue",font=('arial',9,'bold'),relief=RIDGE)
        aux2.grid(column=3, row=9,pady=5)
        aux3 = Label(self.__window, text='''  • Exactly x ''',bg="lightsteelblue",font=('arial',9,'bold'),relief=RIDGE)
        aux3.grid(column=3, row=10,pady=5)
        aux3 = Label(self.__window, text=''' Replacing x with the desired price''', bg="lightsteelblue", font=('arial', 9, 'bold'),relief=RIDGE)
    ...
    def __createRoom(self):
        room = zimmer(self.__Zimmer_nummer_txt.get(),self.__Zimmer_amg_txt.get(),self.__Zimmer_preis_txt.get(),self.__Zimmer_farbe_txt.get(),bool(self.__Zimmer_meerblick_txt.get().lower()=="yes"))
    ...
    def __deleteRoom(self):
        room = zimmer(self.__Zimmer_nummer_txt.get(),self.__Zimmer_amg_txt.get(),self.__Zimmer_preis_txt.get(),self.__Zimmer_farbe_txt.get(),self.__Zimmer_meerblick_txt.get())
        self.__controler.delete_room(room.nummer)

        self.__Zimmer_nummer_txt.delete(0, 'end')

    def __Print(self):
        resultLabel = Label(self.__window, text=str(control.print_rooms()),bg="lightsteelblue",font=('arial',9,'bold'),relief=RIDGE)
    ...
    def __Filter(self):
        ...
        if len(aux) == 0:
            raise ValueError
        resultLabel = Label(self.__window, text=str(control.filter_room(nr, bool(self.__Zimmer_meerblick_txt.get().lower()=="yes"), (lambda: ["moreexpensivethan","lessexpensivethan","exactly"].index(aux) + 1 if aux in ["moreexpensivethan","lessexpensivethan","exactly"] else 4)())),bg="lightsteelblue",font=('arial',9,'bold'),relief=RIDGE)
...
class MenuReservierung:
    def __init__(self, gui_master):
        ...
        zimmerNummer = Label(self.__window, text='Room Number',bg="lightsteelblue",font=('arial',9,'bold'),relief=RIDGE)
        zimmerNummer.grid(column=1, row=4)
        self.__Reserv_zimmerNummer_txt.grid(column=2, row=4)
        anfang = Label(self.__window, text='First Day (dd/mm/yy)',bg="lightsteelblue",font=('arial',9,'bold'),relief=RIDGE)
        anfang.grid(column=1, row=5)
        self.__Reserv_anfang_data_txt.grid(column=2, row=5)
        ende = Label(self.__window, text='Last Day (dd/mm/yy)',bg="lightsteelblue",font=('arial',9,'bold'),relief=RIDGE)
        ende.grid(column=1, row=6)
        self.__Reserv_ende_data_txt.grid(column=2, row=6)

        #butoane pentru no_reserv_guest
        btnR2 = Button(self.__window, text='Print Guests without Reservation', command = self.__PrintNoReserv,bg="lightsteelblue")
    ...
    def __makeReservation(self):
        ...
        txt=self.__controler.make_reservation(rezerv, gast(self.__Reserv_Vorname_txt.get(), self.__Reserv_Nachname_txt.get()))
    ...
    def __PrintNoReserv(self):
        resultLabel2 = Label(self.__window, text=str(control.no_reserv_guest()),font=('arial',9,'bold'),relief=RIDGE,bg="lightsteelblue")
        resultLabel2.grid(row=11,column=1)

    def __AvToday(self):
        resultLabel3 = Label(self.__window, text=str(control.available_today()),bg="lightsteelblue",font=('arial',9,'bold'),relief=RIDGE)
    ...
    def __Cancel(self):
        ...
        if aux in self.__controler.repo.listg:
            ...
            if deprintat=='':
                resultLabel4 = Label(self.__window, text='No reservations', bg="lightsteelblue",font=('arial', 9, 'bold'), relief=RIDGE)
                resultLabel4.grid(row=13, column=1)
            else:
                resultLabel4 = Label(self.__window, text=deprintat, bg="lightsteelblue",font=('arial', 9, 'bold'), relief=RIDGE)
                resultLabel4.grid(row=13, column=1, columnspan=3)
                #7 e gol
                opt = Label(self.__window, text='Nr of reservation to cancel', bg="lightsteelblue", font=('arial', 9, 'bold'),relief=RIDGE)
            ...
            resultLabel4 = Label(self.__window, text='No such guest. Try again!', bg="lightsteelblue",font=('arial', 9, 'bold'), relief=RIDGE)
...
class MenuHELP:
    def __init__(self, gui_master):
        ...
        HTUR=Button(self.__window, text="How to use Reservation Menu?", command=self.new_windowHTUR,bg="lightsteelblue")
        HTUR.pack()
    ...
    def new_windowA(self):
        ...
        lab1= Label(about_window,text="25 Single rooms, 20 Duet rooms and 15 penthouses with seaview",relief=RIDGE,bg="lightsteelblue")
    ...
    def new_windowHTUG(self):
        ...
        oL1 = Label(HTUG_window,
                    text=" In order to use the Guest Menu you have to know a few things about how it is organized and the meaning of each button",relief=RIDGE,bg="lightsteelblue")
        ...
        oL3 = Label(HTUG_window,
                    text="By pressing this button you will start the process of adding a guest in the database of the hotel. But what you must now is that you have to fill the First Name and Last Name gaps firstly",relief=RIDGE,bg="lightsteelblue")
        ...
        oL5 = Label(HTUG_window,
                    text="In order to change the last name of a guest you will have to reintroduce his first and last name and also to fill in the specified area the new family name of the guest",relief=RIDGE,bg="lightsteelblue")
        ...
        oL7 = Label(HTUG_window,
                    text="In order to delete a guest from the database you will only have to enter again his first and last name in the specified areas and then press the button",relief=RIDGE,bg="lightsteelblue")
        ...
        oL9 = Label(HTUG_window,
                    text="This button was created for printing the list of guests that are curently renting a room",relief=RIDGE,bg="lightsteelblue")
        oL9.place(x=120, y=280)
        oL10 = Label(HTUG_window,
                     text="When you quit the menu your data are automatically saved, but for more certainty you should use the Save button",relief=RIDGE,bg="lightsteelblue")
    ...
    def new_windowHTUZ(self):
        ...
        oL1 = Label(HTUZ_window,
                    text=" In order to use the Room Menu you have to know a few things about how it is organized and the meaning of each button:",relief=RIDGE,bg="lightsteelblue")
        oL1.place(x=100, y=100)
        oL2 = Label(HTUZ_window, text="1. Add Room Button:",relief=RIDGE,bg="lightsteelblue")
        oL2.place(x=100, y=140)
        oL3 = Label(HTUZ_window,
                    text="By pressing this button you will start the process of adding a room in the database of the hotel. But what you must now is that you have to fill the Number, Capacity, Price, Colour and Seaview gaps firstly",relief=RIDGE,bg="lightsteelblue")
        oL3.place(x=120, y=160)
        oL4 = Label(HTUZ_window, text="Capacity refers to the 'number of people that can stay in that room'.",relief=RIDGE,bg="lightsteelblue")
        ...
        oL6 = Label(HTUZ_window,
                    text="By pressing this button you will start the process of changing the price of a room that already is in the database of the hotel.",relief=RIDGE,bg="lightsteelblue")
        oL6.place(x=120, y=220)
        oL7 = Label(HTUZ_window,
                    text="In order to do so you have to introduce the number of the room and the new price in the specified gaps firstly ",relief=RIDGE,bg="lightsteelblue")
        oL7.place(x=120, y=240)
        oL8 = Label(HTUZ_window, text="3. Delete room:",relief=RIDGE,bg="lightsteelblue")
        oL8.place(x=100, y=260)
        oL9 = Label(HTUZ_window,
                    text="In order to delete a room from the database you will only have to enter the number of the room in the specified area ",relief=RIDGE,bg="lightsteelblue")
        oL9.place(x=120, y=280)
        oL10 = Label(HTUZ_window, text="4. Print rooms:",relief=RIDGE,bg="lightsteelblue")
        oL10.place(x=100, y=300)
        oL11 = Label(HTUZ_window,
                     text="This button was created for printing the list of rooms that are available for renting or that are already rented",relief=RIDGE,bg="lightsteelblue")
        ...
        oL13 = Label(HTUZ_window,
                     text="In order to filter the list of rooms depending on multiple options you will have to introduce exactly what you are looking for.",relief=RIDGE,bg="lightsteelblue")
        oL13.place(x=120, y=360)
        oL14 = Label(HTUZ_window,
                     text="More exactly you will have to introduce a price in the specified gap, the price option which is specified in the menu, and if the room has or not a seaview.",relief=RIDGE,bg="lightsteelblue")
    ...
    def new_windowHTUR(self):
        ...
        oL3 = Label(HTUR_window,
                    ...
                    relief=RIDGE, bg="lightsteelblue")
        oL3.place(x=120, y=160)
        oL4 = Label(HTUR_window, text=" But what you must now is that you have to fill First Name, Last Name, Room Number, First Day and Last Day gaps firstly",
                    relief=RIDGE, bg="lightsteelblue")
        oL4.place(x=120, y=180)
        oL5 = Label(HTUR_window, text="2. 'Print Guests without Reservation ' Button:", relief=RIDGE, bg="lightsteelblue")
        oL5.place(x=100, y=200)
        ...
        oL8 = Label(HTUR_window, text="This button was created for printing the list of rooms that are available for renting today", relief=RIDGE, bg="lightsteelblue")
